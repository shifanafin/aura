...
export function Skills() {
  ...
  const [positions, setPositions] = useState(skills.map(() => ({ x: Math.random() * 80 - 40, y: Math.random() * 80 - 40 })));

  useEffect(() => {
    const interval = setInterval(() => {
      setPositions(skills.map(() => ({ x: Math.random() * 80 - 40, y: Math.random() * 80 - 40 })));
    }, 3000);
